...
- `dist/` is the only published content (`"files": ["dist"]`)
...
- Alias-import rule and ESLint only run on staged `.js/.jsx` files — `.ts/.tsx` are skipped. Always run `npm run lint` yourself
